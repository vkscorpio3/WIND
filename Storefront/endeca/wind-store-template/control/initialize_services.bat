...
REM import CAS index configuration using public format
echo y|call %~dp0index_config_cmd.bat set-config -f %~dp0..\config\index_config\index-config.json -o all
if not %ERRORLEVEL%==0 (
	echo Failed to import CAS index configuration in public format.
	exit /B 1
)
